  ...
  fetch(SHEET_URL)
    ...
    .then(text => {
      ...
      document.addEventListener('keydown', function(e) {
        if (modal.style.display === 'flex' && currentModalIndex !== -1 && currentGalleryItems.length) {
          if (e.key === 'ArrowRight') {
            currentModalIndex = (currentModalIndex + 1) % currentGalleryItems.length;
            modalImg.src = currentGalleryItems[currentModalIndex].src;
          } else if (e.key === 'ArrowLeft') {
            currentModalIndex = (currentModalIndex - 1 + currentGalleryItems.length) % currentGalleryItems.length;
            modalImg.src = currentGalleryItems[currentModalIndex].src;
